  ...
  <tr key={item.id} className="odd:bg-gray-200 hover:bg-gray-400 transition-colors duration-200">
    <td className="p-2 rounded-l-lg">
      <div className="flex items-center gap-2">
        <div className="w-10 h-10 rounded-full bg-[var(--p)] flex items-center justify-center">
    ...
    <td className="hidden lg:table-cell text-sm capitalize">{item.type}</td>
  ...
  return (
    <div className='bg-[var(--bg)] p-4 rounded-md flex-1 m-3'>
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-2xl font-semibold text-[var(--p)]">All Results</h1>
      ...
      {/* table */}
      <div className="mt-4">
        <Tables columns={columns} renderRows={renderRows} data={resultsData} />
      </div>
    </div>
  )
}
