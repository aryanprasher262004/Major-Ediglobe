...
const renderRows=(item : Event) => (
  <tr key={item.id} className="odd:bg-gray-200 hover:bg-gray-400 transition-colors duration-200">
    ...
    <td className="hidden lg:table-cell text-sm">{item.startTime} - {item.endTime}</td>
  ...
  return (
    <div className='bg-[var(--bg)] p-4 rounded-md flex-1 m-3'>
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-2xl font-semibold text-[var(--p)]">All Events</h1>
      ...
      {/* table */}
      <div className="mt-4">
        <Tables columns={columns} renderRows={renderRows} data={eventsData} />
      </div>
    </div>
  )
}
